...
void R_InitTextures(void) {
  ...
  // Z_Free (names); Don't free, CacheLumpNum just gives us a pointer to the
  // data.
  ...
  for (i = 0; i < numtextures; i++, directory++) {
    ...
    for (j = 0; j < texture->patchcount; j++, mpatch++, patch++) {
      ...
      if (mpatch->patch >= nummappatches) {
        ...
        fprintf(stderr,
                "WARNING WARNING WARNING mpatch->patch %d > %d @ %d / %d\n",
  ...
  // Z_Free (maptex1); Don't free, CacheLumpNum just gives us a pointer to the
  // data.
  //  if (maptex2)
  // Z_Free (maptex2); Don't free, CacheLumpNum just gives us a pointer to the
  // data.
