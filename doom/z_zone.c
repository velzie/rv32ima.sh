...
void *Z_Malloc_Internal_Extended(int size, int tag, void *ptr, const char *fil,
                                 int line) {
  void *ret;
  usedram += size;
#if SET_MEMORY_DEBUG
  printf("MALLOC: %d %d %p @ %s:%d -> %d ->", size, tag, ptr, fil, line,
         usedram);
#endif
...
void *Z_Malloc_Internal(int size, int tag, void *user) {
  int extra;
  memblock_t *start;
  memblock_t *rover;
  memblock_t *newblock;
